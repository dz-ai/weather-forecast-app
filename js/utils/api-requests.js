...
export async function citySearch (cityName) {
   const response = await fetch(urlCity + `q=${cityName}&appid=${secrets.APIKEY}`);
    return await response.json()
}

export async function foreCastSearch (lat, lon, unit) {
  const response = await fetch(urlForecast + `lat=${lat}&lon=${lon}&units=${unit}&appid=${secrets.APIKEY}`);
